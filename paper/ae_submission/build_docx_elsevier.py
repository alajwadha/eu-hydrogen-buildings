...
Out:  paper/ae_submission/V7.docx
"""
...
# Named for its source, so V7.tex, V7.pdf and V7.docx are one set.
OUT = os.path.join(HERE, "V7.docx")
...
# ── front matter, read from V7.tex so it cannot drift from the submission ──────────
def frontmatter() -> dict:
    t = open(os.path.join(HERE, "V7.tex"), encoding="utf-8").read()
    ...
    # address here, so changing either in V7.tex reaches the Word file.
...
# Everything V7.tex prints outside sections/. The Nomenclature sits before the body and
...
def _sections_from_main(names) -> str:
    """Pull named \\section* blocks out of V7.tex, in the order given.

    Taken from V7.tex rather than retyped, so the Word file cannot drift from the PDF.
    ...
    """
    src = open(os.path.join(HERE, "V7.tex"), encoding="utf-8").read()
    out, missing = [], []
    for name in names:
    ...
    if missing:
        print("  NOT FOUND in V7.tex:", ", ".join(missing))
...
def build_source() -> str:
    ...
    # Back matter, lifted verbatim from V7.tex. Without this the Word file ended at the
    ...
    # Resolve \ref and \eqref against V7.aux. Without this pandoc leaks the raw
    ...
    # number comes from the same V7.aux the cross-references resolve against, so the
...
def _aux_labels() -> dict:
    """label -> printed number, read from the compiled V7.aux. Never guessed."""
    path = os.path.join(HERE, "V7.aux")
...
def _number_captions(text: str) -> str:
    """Prefix each float caption with "Figure N: " or "Table N: " from V7.aux."""
    labels = _aux_labels()
...
def set_page_size_letter(d) -> int:
    ...
    elsarticle is loaded with letterpaper, so V7.pdf is 612 x 792 pt, while pandoc's
...
def flatten_empty_base_scripts(d) -> int:
    ...
    Anything that is not Word draws the zero-width space as a box. Rendering V7.docx
