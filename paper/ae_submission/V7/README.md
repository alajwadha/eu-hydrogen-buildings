...
**Upload the files in this folder.** They are copies of `V7.pdf`, `V7.docx`, `SI.pdf`
...
and these are renderings. `package_v7.py` refreshes them from the built deliverables and
`check_submission.py` fails if they differ, so the two cannot drift again. Run the build,
then `python3 package_v7.py`, then the gate.

| File | What it is |
|---|---|
| `Hydrogen_residential_heating_V7.pdf` | Main manuscript, elsarticle `[review]`: single column, double spaced |
| `Hydrogen_residential_heating_V7.docx` | The same manuscript, written natively as Word |
| `Hydrogen_residential_heating_SI_V7.pdf` | Supplementary Information |
| `Hydrogen_residential_heating_graphical_abstract_V7.png` | Graphical abstract, 3984 x 1593 px, a separate Editorial Manager upload |
