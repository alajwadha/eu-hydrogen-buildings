...
broken at least once during this paper's development:
  ...
  * that V7.tex and frontmatter_body.tex still carry a token-identical abstract and
...
DOCS = [(HERE, "V7"), (HERE, "SI"), (REPO / "paper", "Paper_v20")]
...
def check_frontmatter() -> None:
    print("\nFront matter")
    a_main, h_main = frontmatter((HERE / "V7.tex").read_text())
    a_twin, h_twin = frontmatter((HERE / "frontmatter_body.tex").read_text())

    gate(a_main == a_twin, "abstract identical in V7.tex and frontmatter_body.tex")
    gate(h_main == h_twin, "highlights identical in V7.tex and frontmatter_body.tex")
    ...
    m_src = (HERE / "V7.tex").read_text()
    t_src = (HERE / "frontmatter_body.tex").read_text()
    gate(_entries(m_src) == _entries(t_src),
         "nomenclature identical in V7.tex and frontmatter_body.tex")
    kw_m, kw_t = _keywords(m_src), _keywords(t_src)
    gate(kw_m == kw_t, "keywords identical in V7.tex and frontmatter_body.tex",
...
# Applied Energy publishes no hard body limit for a research paper; 8,000 was an
# author-imposed discipline. The V7 clarity revision names the quantity attached to every
# number ("19 heat-pump wins at the low bound and 17 at the high bound" for "19 to 17",
# and the full description of the distribution-excluded sensitivity for "asymmetric
# accounting"), which is longer by construction and worth the words. Raised from 8,000 to
# hold the revised text without cutting analysis the referees asked to see. This cap has
# now moved four times and is a record of the author's intent rather than a real limit.
BODY_WORD_CAP = 8200
                              ...
                              # omitted; 7,900 -> 8,000 pays for the V7 text
...
def check_graphical_abstract() -> None:
    ...
    variant cannot be the one V7.tex includes. What was missing is any gate that the
...
def check_word_fields() -> None:
    ...
    if not docx_path.exists():
        gate(False, "Word twin carries live numbering fields", "V7.docx is missing")
        return
    ...
    gate(ok, f"Word twin carries live numbering fields ({n_seq} SEQ, {n_ref} REF)",
         "" if ok else "V7.aux was probably missing when build_docx_elsevier.py ran, "
...
def check_word_styles() -> None:
    ...
    docx_path = HERE / "V7.docx"
    if not docx_path.exists():
        gate(False, "every style the Word twin names is defined", "V7.docx is missing")
        return
    with zipfile.ZipFile(docx_path) as z:
...
def check_word_table_rules() -> None:
    ...
    if not docx_path.exists() or not body.exists():
        gate(False, "the Word twin's tables carry the source's rules",
             "V7.docx or the assembled source is missing; run build_docx_elsevier.py")
        return
...
def check_word_page_structure() -> None:
    ...
    The expected order is derived from V7.pdf rather than typed here, so the check
    ...
    """
    pdf, docx_path = HERE / "V7.pdf", HERE / "V7.docx"
    if not (pdf.exists() and docx_path.exists()):
        gate(False, "Word twin paginates like the PDF", "V7.pdf or V7.docx is missing")
        return
...
def check_upload_set() -> None:
    ...
    stale rendering. This one compares bytes, and package_v7.py is what fixes it.
    ...
    sys.path.insert(0, str(HERE))
    import package_v7
    missing = [s for s, _ in package_v7.PAIRS if not s.exists()]
    if missing:
        gate(False, "V7 upload set matches the built deliverables",
             f"{len(missing)} deliverable(s) not built")
        return
    st = package_v7.stale()
    gate(not st, "V7 upload set matches the built deliverables",
         ("stale: " + ", ".join(d.name for _, d in st) +
          "  (run python3 package_v7.py)") if st else "")

    orph = package_v7.orphans()
    gate(not orph, "V7 upload folder carries no superseded file",
         (", ".join(p.name for p in orph) +
          "  (run python3 package_v7.py)") if orph else "")
