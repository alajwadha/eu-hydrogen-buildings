...
Swiss case does so once the seasonal store is charged on the base-load comparison as well as on the peak. The
...
0.6~per cent. The symmetric pair is what we report, because a comparison that charges
...
two sides are not taxed alike. It does not decide the base-load result, since the heat pump leads in 22 of 29 countries, but it sets how large that lead is and it
...
The two frameworks still supply a cross-check, and a narrow one. Hydrogen remains negligible in both, at a scenario-set share in the simulations
...
removing fossil boilers, short of economy-wide net-zero for the heat supplied. The boundary matters less to the mix than to the cost comparison, but it is
not costless.
...
which countries bind and by how much. We therefore do
not claim the boundary is immaterial. The headline verdict does not rest on it. It rests on the missing-money problem, which is a capital-recovery result independent of
...
(Appendix~\ref{sec:sensitivity:rho}). That invariance buys less than it appears to.
It does not confirm a hydrogen cost disadvantage holding country by
country, since the disadvantage holds in 22 of the 29 countries. What it establishes is
the weaker statement that $\rho$ leaves each country's own cost comparison, whichever way
it falls, exactly where it was.
...
Switzerland could be treated through a one-directional exogenous sensitivity
...
them. We use an \emph{endogenous} integration in place of that one-way coupling,
...
boiler on the uncoupled arm and \euro{}9.6/MWh below it on the endogenous one. A Swiss base-load reversal in hydrogen's
favour is an artefact of omitting the seasonal store from the base-load
comparison while charging it in the peak and dispatch tests, and it does not survive
charging the store consistently.
...
The derate does, however, move the building-peak count, and we sweep it here. $\mathrm{COP}^{\mathrm{cold}}=\max(f\,\mathrm{SCOP},\text{floor})$ is published at
...
electrification requires is charged in the headline levelised cost of heat, for
...
its own last-mile network on symmetric terms.
