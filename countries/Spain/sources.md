...
- Spain-specific bioenergy and heating data
...
Sources for the bottom-up heat-demand build (`es.yaml`, methodology in
`literature/spain/classification_methodology.md`):
...
- **Spanish TABULA typology (CIEMAT / IVE-Valencia)** — per-class × per-cohort space-heating energy need. https://episcope.eu/building-typology/country/es/ ; IVE *Catalogo de Tipologias Residenciales de Espana*. See `code/data/raw/tabula/es_intensities.csv` (research-synthesised pending WebTool verification).
- **Eurostat `nrg_chdd_a`** — heating degree days (base 15 °C, JRC AGRI4CAST); 2022 confirmed 1478, 5-year mean estimated 1750. https://ec.europa.eu/eurostat/databrowser/view/nrg_chdd_a
- **IDAE / Odyssee-Mure Spain** — retrofit-state assumptions, PAREER / RRF Component 2 reporting.
